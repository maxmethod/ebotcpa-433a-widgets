/**
 * 433b-payment-acceptance embed bootstrap
 * Generated from 433b-payment-acceptance.html by scripts/build-embed.js — do not hand-edit.
 *
 * Drop into a GHL form step (Custom Code element) with:
 *   <div id="payment-433b-widget" data-primary-color="{{custom_values.brand_primary_color}}"></div>
 *   <script src="https://cdn.jsdelivr.net/gh/maxmethod/ebotcpa-433a-widgets@vX.Y.Z/dist/embed-433b-payment-acceptance.js"></script>
 *
 * Or omit the <div> and the script appends the widget to <body>.
 */
(function () {
  if (window.__payment433bEmbedLoaded) return;
  window.__payment433bEmbedLoaded = true;

  // ---- styles ----
  const style = document.createElement('style');
  style.setAttribute('data-payment-433b', 'styles');
  ...
  }#payment-433b-widget#payment-433b-widget * { box-sizing: border-box; }#payment-433b-widget#payment-433b-widget .ar-container { font-family: var(--ar-font); color: var(--ar-text); font-size: 15px; line-height: 1.5; }#payment-433b-widget#payment-433b-widget .ar-section { background: transparent; border: none; border-radius: 0; padding: 0; margin-bottom: 22px; }#payment-433b-widget#payment-433b-widget .ar-section + .ar-section { padding-top: 22px; border-top: 1px solid var(--ar-border); }#payment-433b-widget#payment-433b-widget .ar-section-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; }#payment-433b-widget#payment-433b-widget .ar-section-title { font-size: 16px; font-weight: 700; margin: 0; }#payment-433b-widget#payment-433b-widget .ar-section-count { color: var(--ar-text-muted); font-size: 13px; }#payment-433b-widget#payment-433b-widget .ar-subtitle { font-size: 12px; color: var(--ar-text-muted); margin: 0 0 14px; }#payment-433b-widget#payment-433b-widget label { display: block; font-size: 13px; font-weight: 500; margin-bottom: 6px; color: var(--ar-text); }#payment-433b-widget#payment-433b-widget input[type="text"] {
  ...
  }#payment-433b-widget#payment-433b-widget input:focus { outline: none; border-color: var(--ar-accent); box-shadow: 0 0 0 3px color-mix(in srgb, var(--ar-accent) 22%, transparent); }#payment-433b-widget#payment-433b-widget .ar-amount-wrap { position: relative; }#payment-433b-widget#payment-433b-widget .ar-amount-wrap input { padding-left: 22px; }#payment-433b-widget#payment-433b-widget .ar-amount-wrap::before { content: "$"; position: absolute; left: 10px; top: 50%; transform: translateY(-50%); color: var(--ar-text-muted); font-size: 14px; pointer-events: none; }#payment-433b-widget#payment-433b-widget button {
  ...
  }#payment-433b-widget#payment-433b-widget button:hover { background: var(--ar-surface); }#payment-433b-widget#payment-433b-widget button.ar-primary { background: var(--ar-accent); border-color: var(--ar-accent); color: #fff; }#payment-433b-widget#payment-433b-widget button.ar-primary:hover { background: var(--ar-accent-hover); border-color: var(--ar-accent-hover); }#payment-433b-widget#payment-433b-widget button.ar-danger-text { background: transparent; border: none; color: var(--ar-danger); padding: 4px 8px; font-size: 13px; }#payment-433b-widget#payment-433b-widget button.ar-danger-text:hover { color: var(--ar-danger-hover); background: transparent; }#payment-433b-widget#payment-433b-widget .ar-add-box { background: #fff; border: 1px dashed var(--ar-border-strong); border-radius: var(--ar-radius); padding: 14px; }#payment-433b-widget#payment-433b-widget .ar-add-box h4 { margin: 0 0 10px; font-size: 14px; font-weight: 600; }#payment-433b-widget#payment-433b-widget .ar-field-group { display: grid; grid-template-columns: repeat(auto-fill, minmax(var(--ar-col-min, 240px), 1fr)); gap: 14px 16px; margin-bottom: 12px; }#payment-433b-widget#payment-433b-widget .ar-field-group > div { display: flex; flex-direction: column; justify-content: flex-end; }#payment-433b-widget#payment-433b-widget .ar-actions-right { display: flex; justify-content: flex-end; gap: 8px; margin-top: 4px; }#payment-433b-widget#payment-433b-widget .ar-items-list { margin-top: 14px; display: flex; flex-direction: column; gap: 8px; }#payment-433b-widget#payment-433b-widget .ar-item-card { background: #fff; border: 1px solid var(--ar-border); border-radius: var(--ar-radius); padding: 12px 14px; display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; }#payment-433b-widget#payment-433b-widget .ar-item-main { flex: 1; min-width: 0; }#payment-433b-widget#payment-433b-widget .ar-item-name { font-weight: 500; font-size: 14px; }#payment-433b-widget#payment-433b-widget .ar-item-meta { font-size: 13px; color: var(--ar-text-muted); margin-top: 2px; }#payment-433b-widget#payment-433b-widget .ar-item-amount { font-weight: 600; font-size: 14px; white-space: nowrap; text-align: right; }#payment-433b-widget#payment-433b-widget .ar-empty { font-size: 13px; color: var(--ar-text-muted); font-style: italic; padding: 6px 0 0; }#payment-433b-widget#payment-433b-widget .ar-total { display: flex; justify-content: space-between; align-items: baseline; margin-top: 12px; padding-top: 10px; border-top: 2px solid var(--ar-border); font-size: 14px; font-weight: 700; }#payment-433b-widget#payment-433b-widget .ar-total .ar-total-val { color: var(--ar-accent); font-size: 16px; font-variant-numeric: tabular-nums; }#payment-433b-widget#payment-433b-widget .ar-warn { background: #fff3cd; border: 1px solid #ffd866; color: #665300; padding: 8px 12px; border-radius: var(--ar-radius); font-size: 13px; margin-bottom: 10px; display: none; }#payment-433b-widget#payment-433b-widget .ar-warn.visible { display: block; }`;
  document.head.appendChild(style);

  // ---- markup ----
  let container = document.getElementById('payment-433b-widget') ||
                  document.querySelector('[data-payment-433b-widget]');
  if (!container) {
    container = document.createElement('div');
    container.id = 'payment-433b-widget';
...
(function () {
  ...
  // ============================================================
  var CONFIG = {
    "containerId": "payment-433b-widget",
    "configGlobal": "PAYMENT_433B_CONFIG",
    "sections": [
      {
        "key": "433b__payment_processors_summary",
        "id": "ldKHHNXWbhf6t0PKQuy0",
        ...
        "addLabel": "Add a payment processor",
        "addBtn": "Add processor",
        "gateWarn": "Enter the processor name before adding.",
        "emptyNote": "No payment processors added yet.",
        "columns": [
          {
            "id": "name",
            "label": "Processor name & address",
            "kind": "text",
      ...
      {
        "key": "433b__credit_cards_summary",
        "id": "oDNElyzSXpRiwGFMWfzB",
        "title": "Credit Cards Accepted",
        "noTotal": true,
        "subtitle": "Types of credit cards the business accepts (IRS Form 433-B, line 6).",
        "addLabel": "Add a credit card type",
        "addBtn": "Add card type",
        "gateWarn": "Enter the type of card before adding.",
        "emptyNote": "No credit card types added yet.",
        "columns": [
          {
            "id": "type",
            "label": "Type of card (Visa, Mastercard…)",
            "kind": "text",
            "ph": "e.g. Visa, Mastercard, Amex",
            "gate": true,
            "role": "name"
          },
          {
          ...
          {
            "id": "bank",
            "label": "Issuing bank name & address",
            "kind": "text",
            "ph": "Name, street, city, state, ZIP",
            "slabel": "Bank"
          },
