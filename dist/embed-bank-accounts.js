...
(function () {
  ...
  }#bank-433a-widget#bank-433a-widget * { box-sizing: border-box; }#bank-433a-widget#bank-433a-widget .ar-container { font-family: var(--ar-font); color: var(--ar-text); font-size: 15px; line-height: 1.5; }#bank-433a-widget#bank-433a-widget .ar-section {
    background: var(--ar-surface);
    border: 1px solid var(--ar-border);
    border-radius: var(--ar-radius);
    padding: 18px 20px;
    margin-bottom: 18px;
  }#bank-433a-widget#bank-433a-widget .ar-section-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; }#bank-433a-widget#bank-433a-widget .ar-section-title { font-size: 16px; font-weight: 600; margin: 0; }#bank-433a-widget#bank-433a-widget .ar-section-count { color: var(--ar-text-muted); font-size: 13px; }#bank-433a-widget#bank-433a-widget label { display: block; font-size: 13px; font-weight: 500; margin-bottom: 6px; color: var(--ar-text); }#bank-433a-widget#bank-433a-widget input[type="text"] {
  ...
  }#bank-433a-widget#bank-433a-widget input:focus {
    outline: none; border-color: var(--ar-accent);
    box-shadow: 0 0 0 3px color-mix(in srgb, var(--ar-accent) 22%, transparent);
  }#bank-433a-widget#bank-433a-widget .ar-amount-wrap { position: relative; }#bank-433a-widget#bank-433a-widget .ar-amount-wrap input { padding-left: 22px; }#bank-433a-widget#bank-433a-widget .ar-amount-wrap::before {
    content: "$"; position: absolute; left: 10px; top: 50%; transform: translateY(-50%);
    color: var(--ar-text-muted); font-size: 14px; pointer-events: none;
  }#bank-433a-widget#bank-433a-widget button {
  ...
  }#bank-433a-widget#bank-433a-widget .ar-item-main { flex: 1; min-width: 0; }#bank-433a-widget#bank-433a-widget .ar-item-name { font-weight: 500; font-size: 14px; }#bank-433a-widget#bank-433a-widget .ar-item-meta { font-size: 13px; color: var(--ar-text-muted); margin-top: 2px; }#bank-433a-widget#bank-433a-widget .ar-item-amount { font-weight: 600; font-size: 14px; white-space: nowrap; text-align: right; }#bank-433a-widget#bank-433a-widget .ar-empty { font-size: 13px; color: var(--ar-text-muted); font-style: italic; padding: 6px 0 0; }#bank-433a-widget#bank-433a-widget .ar-warn {
  ...
  }#bank-433a-widget#bank-433a-widget .ar-warn.visible { display: block; }`;
...
(function () {
  ...
  // Override via window.BANK_433A_CONFIG = { fieldKeys: {...}, primaryColor: '#...' }.
  // ============================================================
  var CFG = (typeof window !== 'undefined' && window.BANK_433A_CONFIG) || {};
  var FIELD_KEYS = Object.assign({ bank: ['433a__bank_accounts_summary'] }, CFG.fieldKeys || {});
  var MAX = 10;

  var widgetEl = document.getElementById('bank-433a-widget');
  var state = { banks: [] };
  ...
  function buildBankSummary() {
    return state.banks.map(function (b) {
      return [sanitize(b.name), sanitize(b.type), sanitize(money(b.balance))].join(' | ');
    }).join('\n');
  }
  function sync(write) { if (!write) return; setAll(FIELD_KEYS.bank, buildBankSummary()); }
  ...
  function hideHostFields() {
    FIELD_KEYS.bank.forEach(function (key) {
      document.querySelectorAll('[name="' + key + '"], [data-q="' + key + '"]').forEach(function (el) {
  ...
  // ============================================================
  // BRAND PRIMARY COLOR — 1) window.BANK_433A_CONFIG.primaryColor,
  ...
  function reconcileFallbacks() {
    ...
    FIELD_KEYS.bank.forEach(function (key) {
      ...
      if (real && fb) { fb.parentNode.removeChild(fb); setAll([key], buildBankSummary()); } // real host appeared -> drop the dup, sync the real field
  ...
  // who fills the fields but forgets to click "Add account" doesn't lose the row.
  // ============================================================
  function flushPending() { if (bankEls.name.value.trim()) { var b = $('bank-add'); if (b) b.click(); } }
